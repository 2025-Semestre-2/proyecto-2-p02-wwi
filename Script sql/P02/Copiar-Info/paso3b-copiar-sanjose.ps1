# Script: Copiar Datos a WWI_Sucursal_SJ (San Jose) - CON FRAGMENTACION HORIZONTAL
# Copia datos con fragmentacion por StateProvinceID <= 26
# Basado en paso3a-copiar-corporativo-v2.ps1 (version exitosa)

Write-Host "====================================================================" -ForegroundColor Cyan
Write-Host "  COPIAR DATOS - WWI_SUCURSAL_SJ (SAN JOSE)" -ForegroundColor Cyan
Write-Host "  CON FRAGMENTACION HORIZONTAL: StateProvinceID <= 26" -ForegroundColor Cyan
Write-Host "  Origen: localhost,1540 (WideWorldImporters P1)" -ForegroundColor Cyan
Write-Host "  Destino: localhost,1434 (WWI_Sucursal_SJ P2)" -ForegroundColor Cyan
Write-Host "====================================================================" -ForegroundColor Cyan
Write-Host ""
...
$servidorDestino = "localhost,1434"
$dbDestino = "WWI_Sucursal_SJ"
$passwordDestino = "WideWorld2024!"

$tempDir = "$env:TEMP\WWI_Transfer_SanJose"
$logFile = "$PSScriptRoot\copiar-sanjose.log"
...
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
Log-Message "PASO 0: Preparar base de datos San Jose" "Cyan"
Log-Message "====================================================================" "Cyan"
...
PRINT 'Base de datos preparada para San Jose';
GO
"@
...
# PASO 1: Copiar tablas BASE (sin dependencias) - COMPLETAS
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
...
# PASO 2: Copiar tablas NIVEL 1 (dependen de base) - COMPLETAS
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
...
# PASO 3: Copiar tablas NIVEL 2 - COMPLETAS
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
...
$tablasNivel2 = @(
    ...
    @{Schema="Application"; Tabla="People"}  # Sin campos sensibles (ya en estructura)
)
...
# PASO 4: Copiar tablas NIVEL 3 - COMPLETAS (proveedores y productos)
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
...
# PASO 5: Copiar tablas NIVEL 4 - COMPLETAS
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
...
# ===========================================================================================
# PASO 6: FRAGMENTACION HORIZONTAL - CLIENTES (StateProvinceID <= 26)
# ===========================================================================================
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
Log-Message "PASO 6: FRAGMENTACION - Clientes de San Jose (StateProvinceID <= 26)" "Cyan"
Log-Message "====================================================================" "Cyan"

$filtroSanJose = "DeliveryCityID IN (SELECT CityID FROM Application.Cities WHERE StateProvinceID <= 26)"

if (Process-Table-Filtered "Sales" "Customers" $filtroSanJose) {
    Log-Message "OK: Sales.Customers fragmentado (San Jose)" "Green"
} else {
    Log-Message "FALLO: Sales.Customers" "Red"
}

# PASO 7: FRAGMENTACION - Ordenes de clientes de San Jose
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
Log-Message "PASO 7: FRAGMENTACION - Ordenes de San Jose" "Cyan"
Log-Message "====================================================================" "Cyan"

$filtroOrders = "CustomerID IN (SELECT CustomerID FROM Sales.Customers WHERE DeliveryCityID IN (SELECT CityID FROM Application.Cities WHERE StateProvinceID <= 26))"

if (Process-Table-Filtered "Sales" "Orders" $filtroOrders) {
    Log-Message "OK: Sales.Orders fragmentado (San Jose)" "Green"
} else {
    Log-Message "FALLO: Sales.Orders" "Red"
}

# PASO 8: FRAGMENTACION - Facturas de clientes de San Jose
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
Log-Message "PASO 8: FRAGMENTACION - Facturas de San Jose" "Cyan"
Log-Message "====================================================================" "Cyan"

$filtroInvoices = "CustomerID IN (SELECT CustomerID FROM Sales.Customers WHERE DeliveryCityID IN (SELECT CityID FROM Application.Cities WHERE StateProvinceID <= 26))"

if (Process-Table-Filtered "Sales" "Invoices" $filtroInvoices) {
    Log-Message "OK: Sales.Invoices fragmentado (San Jose)" "Green"
} else {
    Log-Message "FALLO: Sales.Invoices" "Red"
}

# PASO 9: FRAGMENTACION - Lineas de ordenes
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
Log-Message "PASO 9: FRAGMENTACION - Lineas de Ordenes de San Jose" "Cyan"
Log-Message "====================================================================" "Cyan"

$filtroOrderLines = "OrderID IN (SELECT OrderID FROM Sales.Orders WHERE CustomerID IN (SELECT CustomerID FROM Sales.Customers WHERE DeliveryCityID IN (SELECT CityID FROM Application.Cities WHERE StateProvinceID <= 26)))"

if (Process-Table-Filtered "Sales" "OrderLines" $filtroOrderLines) {
    Log-Message "OK: Sales.OrderLines fragmentado (San Jose)" "Green"
} else {
    Log-Message "FALLO: Sales.OrderLines" "Red"
}

# PASO 10: FRAGMENTACION - Lineas de facturas
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
Log-Message "PASO 10: FRAGMENTACION - Lineas de Facturas de San Jose" "Cyan"
Log-Message "====================================================================" "Cyan"

$filtroInvoiceLines = "InvoiceID IN (SELECT InvoiceID FROM Sales.Invoices WHERE CustomerID IN (SELECT CustomerID FROM Sales.Customers WHERE DeliveryCityID IN (SELECT CityID FROM Application.Cities WHERE StateProvinceID <= 26)))"

if (Process-Table-Filtered "Sales" "InvoiceLines" $filtroInvoiceLines) {
    Log-Message "OK: Sales.InvoiceLines fragmentado (San Jose)" "Green"
} else {
    Log-Message "FALLO: Sales.InvoiceLines" "Red"
}

# PASO 11: FRAGMENTACION - Transacciones de clientes
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
Log-Message "PASO 11: FRAGMENTACION - Transacciones de Clientes de San Jose" "Cyan"
Log-Message "====================================================================" "Cyan"

$filtroCustomerTransactions = "CustomerID IN (SELECT CustomerID FROM Sales.Customers WHERE DeliveryCityID IN (SELECT CityID FROM Application.Cities WHERE StateProvinceID <= 26))"

if (Process-Table-Filtered "Sales" "CustomerTransactions" $filtroCustomerTransactions) {
    Log-Message "OK: Sales.CustomerTransactions fragmentado (San Jose)" "Green"
} else {
    Log-Message "FALLO: Sales.CustomerTransactions" "Red"
}

# PASO 12: FRAGMENTACION - Special Deals (si aplica)
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
Log-Message "PASO 12: FRAGMENTACION - Special Deals de San Jose" "Cyan"
Log-Message "====================================================================" "Cyan"

$filtroSpecialDeals = "CustomerID IN (SELECT CustomerID FROM Sales.Customers WHERE DeliveryCityID IN (SELECT CityID FROM Application.Cities WHERE StateProvinceID <= 26))"

if (Process-Table-Filtered "Sales" "SpecialDeals" $filtroSpecialDeals) {
    Log-Message "OK: Sales.SpecialDeals fragmentado (San Jose)" "Green"
} else {
    Log-Message "FALLO: Sales.SpecialDeals" "Red"
}

# PASO 13: Copiar PurchaseOrders - COMPLETO (NO fragmentar)
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
...
# PASO 14: Copiar PurchaseOrderLines - COMPLETO
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
...
# PASO 15: Copiar SupplierTransactions - COMPLETO
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
...
# PASO 16: FRAGMENTACION - StockItemTransactions (por CustomerID cuando existe)
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
Log-Message "PASO 16: FRAGMENTACION - StockItemTransactions de San Jose" "Cyan"
Log-Message "====================================================================" "Cyan"

$filtroStockTrans = "CustomerID IN (SELECT CustomerID FROM Sales.Customers WHERE DeliveryCityID IN (SELECT CityID FROM Application.Cities WHERE StateProvinceID <= 26)) OR CustomerID IS NULL"

if (Process-Table-Filtered "Warehouse" "StockItemTransactions" $filtroStockTrans) {
    Log-Message "OK: Warehouse.StockItemTransactions fragmentado (San Jose + NULL)" "Green"
} else {
...
Log-Message "" "White"
Log-Message "====================================================================" "Cyan"
Log-Message "VERIFICACION - Datos copiados a San Jose" "Cyan"
Log-Message "====================================================================" "Cyan"
...
PRINT '';
PRINT '=== RESUMEN DE DATOS EN SAN JOSE ===';
PRINT '';
...
PRINT '';
PRINT '=== VERIFICACION: Clientes deben tener StateProvinceID <= 26 ===';
SELECT 
...
$totalExitosos = $exitososBase + $exitososNivel1 + $exitososNivel2 + $exitososNivel3 + $exitososNivel4 + 12  # +12 tablas fragmentadas
...
Log-Message "" "White"
Log-Message "Fragmentacion: StateProvinceID <= 26 (San Jose)" "Cyan"
